...
double pi = 3.14159265358979323846; //n�mero pi
...
double V(double x)
// Funcion potencial  en eV Se toma el m�nimo como cero
//retorna 2mV(x)/h^2
{
  if (x >= a && x <= b)
...
{ /* Metodo de diferencias finitas.
  ...
  */
  ...
  /* L�mites de x para calcular la FdO
 la mitad de los puntos dentro del pozo y la mitad fuera */
  ...
  printf(" POZO CUADRADO");
  fprintf(pfileout, " POZO CUADRADO");
  ...
  fprintf(pfileout, "\n Dx: %lf A, no. de puntos calculados de Fi: %i", Dx, n);
  printf("\n Lista la funci�n de ona n�mero: %i", nf);
  fprintf(pfileout, "\n Lista la funci�n de ona n�mero: %i", nf);
 ...
 z= matriz Para convertir matriz sim�trica en tridiagonal. 
...
*/
  ...
  */
  ...
  En este programa s�lo  lista los autovalores y un autovector
 */
    z[i + n * i] = 1.;
    EeV = V(xi);
...
*/
  ...
  for (i = 0; i < ilist; i++)
  {
    EeV = d[i] / Dx / Dx / gamma;
    EdivE0 = d[i] / d[0];
    //Energ�as para el pozo infinito en eV (calculo analitico para comparar)
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (Phi2_max < z[i + nf * n] * z[i + nf * n])
      Phi2_max = z[i + nf * n] * z[i + nf * n];
  }
  ...
  for (i = 0; i < n; i++)
  {
    ...
    x = (i - n / 4.) * Dx + a;
    if (x >= a && x <= b)
      zteo = sqrt(2. / (b - a)) * sin((nf + 1) * pi * (x - a) / (b - a));
    else
      zteo = 0.;
