...
{
    if (x >= conf->a)
        return conf->V0 * x; // Pozo cuadrado  de profundidad V0 en eV.
    else
        return 20;
}
...
#define n (conf->n)
    ...
    FILE *diagonal_matriz_out_file = fopen("Schr_pozo_matriz.txt", "a");;
    ...
    char file_name[30];
    sprintf(file_name, "Schr_pozo_FdO_nivel_%d.txt", nivel_elegido);
    FILE *funcion_onda_file = fopen(file_name, "w");
